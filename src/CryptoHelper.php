...
class CryptoHelper
{
  ...
  public static function encrypt($data, $cert, $cipher = OPENSSL_CIPHER_3DES)
  {
    ...
    $openssl_cipher = 'OPENSSL_CIPHER_' . strtoupper($cipher);
    if (is_string($cipher)) {
      if (!defined($openssl_cipher)) {
        $openssl_cipher = str_replace('-', '_', $openssl_cipher);
        if (!defined($openssl_cipher)) {
          $openssl_cipher .= '_CBC';
        }
      }
      ...
      if (defined($openssl_cipher)) {
        $cipher = constant($openssl_cipher);
      } else {
        throw new \RuntimeException('Failed to encrypt S/Mime message. Error: Unknown encryption algorithm: ' . $cipher);
      }
    }
