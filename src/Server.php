...
class Server
{
  ...
  public function execute(ServerRequestInterface $request = null) : ?Response
  {
    ...
    try {
      ...
      $payload = $this->createBase64MimePart($request);
      ...
      if ($isPayloadSigned && Management::MIC_OF_COMPRESSED_DATA) {
      ...
      //calculate the MIC after decompression (tiamo didäääää so)
      if ($isPayloadSigned && !Management::MIC_OF_COMPRESSED_DATA) {
        $message->setMic(CryptoHelper::calculateMIC($payload, $micalg));
      }
   ...
   * @return MimePart */
  protected function createBase64MimePart(ServerRequestInterface &$request) : MimePart {
